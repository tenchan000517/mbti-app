...
【UI/UXコンセプト】
- engineer-courseプロジェクト（C:/portfolio-course/engineer-course/）のUI概念を参考
- ただしマークダウンは部分的に使用（記事ベースではないため）

【進捗管理】
- このHANDOFF.mdのチェックリストを更新しながら実装

まずはフェーズ1（基盤構築）から始めてください。
特に質問データと16タイプの説明は、Web検索で正確な情報を収集してから作成してください。
```
